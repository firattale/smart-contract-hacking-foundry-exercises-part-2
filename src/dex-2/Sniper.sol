...
contract Sniper is Ownable {
    ...
    // Every failed attempt we will increase the slippage by 0.3% (3 / 1000)
    ...
    function snipe(
        ...
        require(pair != address(0), "The pair has not been created yet");
        ...
        // NOTE: We're using block to avoid "stack too deep" error
        {
            ...
            if (amountOut < _absoluteMinAmountOut) {
                revert("Amount out is lower than minimum absolute amount out");
            }
    ...
    function _swapWithRetries(
        ...
        uint256 slippageTolerance = INITIAL_SLIPPAGE - (SLIPPAGE_INCREMENTS * _retryNo);

        // TODO: Revert if we reached max retries
        require(_retryNo <= _maxRetries, "Max Retry reached");

        // TODO: Set the slippage tolerance based on the _retryNo
        // TODO: Start from INITIAL_SLIPPAGE, then every retry we reduce SLIPPAGE_INCREMENTS
        // TODO: Apply the slippage to the amounts
        uint256 amount0Out = _amount0Out * (slippageTolerance / 1000);
        uint256 amount1Out = _amount1Out * (slippageTolerance / 1000);
        ...
        try IUniswapV2Pair(_pair).swap(amount0Out, amount1Out, address(this), "") {}
        catch {
