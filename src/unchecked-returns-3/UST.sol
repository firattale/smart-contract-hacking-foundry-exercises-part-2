...
abstract contract BasicToken is Ownable, ERC20Basic {
    ...
    */
    function transfer(address _to, uint _value) public onlyPayloadSize(2 * 32) virtual override returns (bool) {
    ...
    */
    function balanceOf(address _owner) override virtual public returns (uint) {
...
abstract contract StandardToken is BasicToken, ERC20 {
    ...
    */
    function transferFrom(address _from, address _to, uint _value) public virtual override onlyPayloadSize(3 * 32) returns (bool){
        ...
        if (fee > 0) {
            // Removed fee transfer to owner
            // balances[address(this).owner()] = balances[address(this).owner()].add(fee);
            // Transfer(_from, address(this).owner(), fee);
        }
    ...
    */
    function approve(address _spender, uint _value) public onlyPayloadSize(2 * 32) virtual override {
    ...
    */
    function allowance(address _owner, address _spender) public virtual override returns (uint) {
...
contract UST is Pausable, StandardToken, BlackList {
    using SafeMath for uint;
    string public name;
    ...
    function transfer(address _to, uint _value) public virtual override(BasicToken, ERC20Basic) whenNotPaused returns (bool) {
        require(!isBlackListed[msg.sender]);
        return super.transfer(_to, _value);
    }

    // Forward ERC20 methods to upgraded contract if this one is deprecated
    function transferFrom(address _from, address _to, uint _value) public virtual override whenNotPaused returns (bool) {
        require(!isBlackListed[_from]);
        return super.transferFrom(_from, _to, _value);
    }

    // Forward ERC20 methods to upgraded contract if this one is deprecated
    function balanceOf(address who) public virtual override(BasicToken, ERC20Basic) returns (uint) {
        if (deprecated) {
    ...
    function totalSupply() public virtual override returns (uint) {
        if (deprecated) {
