...
contract SecuredCryptoKeeper is ICryptoKeeper, ERC165, ERC721Holder, ERC1155Holder, ReentrancyGuard {
    ...
    function initialize(address[] calldata _operators) external {
        ...
        require(_operators.length <= 10, "Can't set more than 10 operators at once");
    ...
    function execute(address _destAddress, bytes calldata _encodedCalldata, uint8 operation)
        external
        isInitialized
    ...
    function executeWithValue(address _destAddress, bytes calldata _encodedCalldata, uint256 _value)
        external
        payable
        isInitialized
        onlyOperator(msg.sender)
    ...
    function _execute(address _destAddress, bytes calldata _encodedCalldata, uint256 _value, uint8 operation)
        private
        nonReentrant
        returns (bytes memory)
    {
        ...
        if (operation == CALL_OPERATION) {
            (success, result) = _destAddress.call{value: _value}(_encodedCalldata);
        ...
        emit Executed(msg.sender, _destAddress, _encodedCalldata, _value, result);
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == type(ICryptoKeeper).interfaceId || super.supportsInterface(interfaceId);
    }
